...
export PEER1_WHOLESELLER_CA=${PWD}/organizations/peerOrganizations/wholeseller.example.com/tlsca/tlsca.wholeseller.example.com-cert.pem
...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_LOCALMSPID="WholesellerMSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER1_WHOLESELLER_CA
    export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/wholeseller.example.com/users/Admin@wholeseller.example.com/msp
...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_ADDRESS=peer1.producer.example.com:7151
  elif [ $USING_ORG -eq 3 ]; then
    export CORE_PEER_ADDRESS=peer0.supplier.example.com:7251
  elif [ $USING_ORG -eq 4 ]; then
    export CORE_PEER_ADDRESS=peer1.supplier.example.com:7351
  elif [ $USING_ORG -eq 5 ]; then
    export CORE_PEER_ADDRESS=peer0.wholeseller.example.com:7451
  elif [ $USING_ORG -eq 6 ]; then
    export CORE_PEER_ADDRESS=peer1.wholeseller.example.com:7551
  else
    errorln "ORG Unknown"
  fi
}
...
parsePeerConnectionParameters() {
  ...
  while [ "$#" -gt 0 ]; do
    ...
    if [ $USING_ORG -eq 1 ]; then
      ...
      PEER="peer1.wholeseller"
      CA=PEER1_WHOLESELLER_CA
    # elif [ $USING_ORG -eq 3 ]; then
    #   PEER="peer0.org3"
    #   CA=PEER0_ORG3_CA
    else
