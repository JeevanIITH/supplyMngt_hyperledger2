...
# NOTE: this must be run in a CLI container since it requires configtxlator
  # peer channel fetch config config_block.pb -o orderer2.example.com:7150 --ordererTLSHostnameOverride orderer2.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  # peer channel fetch config config_block.pb -o orderer3.example.com:7250 --ordererTLSHostnameOverride orderer3.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer1.example.com:7050 --ordererTLSHostnameOverride orderer1.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
